...
def categorize_with_embeddings(result: dict, startup_name: str) -> tuple[str, float]:
    ...
    # Keyword-based categorization (as fallback before embeddings)
    
    # Product indicators in title or snippet
    if any(kw in title or kw in snippet for kw in ["product", "features", "pricing", "how it works", "demo", "use cases"]):
        return "product", 0.9
    
    # Market indicators
    if any(kw in title or kw in snippet for kw in ["market", "industry", "competitors", "competing", "alternative to"]):
        return "market", 0.9
    
    # Traction indicators
    if any(kw in title or kw in snippet for kw in ["growth", "users", "customers", "adoption", "metrics", "revenue"]):
        return "traction", 0.9
