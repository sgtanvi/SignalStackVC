...
SERPAPI_DELAY = 2  # Delay between SerpAPI calls in seconds
...
def categorize_with_embeddings(result: dict, startup_name: str) -> tuple[str, float]:
    ...
    # 1. Entity Disambiguation - Block noise patterns
    BLOCKED_PATTERNS = ["login", "signup", "airdrops", "price today", "compare", "token", "explorer", "coin", "crypto", "chart", "live price", "market cap"]
    if any(p in url for p in BLOCKED_PATTERNS) or any(p in title for p in BLOCKED_PATTERNS):
        return "other", 0.0
    
    # 2. Strong match detection for early exit on misfires
    def is_strong_match(title: str, company_name: str) -> bool:
        """Check if the title strongly matches the company name."""
        title_lower = title.lower()
        company_lower = company_name.lower()
        return (company_lower in title_lower or 
                title_lower.startswith(company_lower) or
                title_lower.endswith(company_lower))
    
    # If it's not a strong match and contains common noise words, filter out
    NOISE_WORDS = ["token", "coin", "crypto", "airdrop", "price", "chart", "trading"]
    if not is_strong_match(title, startup_name) and any(word in title for word in NOISE_WORDS):
        return "other", 0.0
...
def scrape_startup_data(startup_name: str, queries: list[str], results_per_query: int = 10, enrich_content: bool = False) -> dict:
    ...
    for query in queries:
        ...
        # Rate limiting: Wait between queries to avoid hitting rate limits
        if query != queries[-1]:  # Don't wait after the last query
            print(f"Waiting {SERPAPI_DELAY} seconds before next query...")
...
def generate_startup_profile(startup_name: str, data: dict) -> dict:
    """
    ...
    for category, results in categorized_data.items():
        if results:
            ...
            # Reduce snippet repetition - only include top 1-2 entries per category
            max_entries = 1 if category == "other" else 2
    ...
    try:
        ...
        # Extract real sources from high-confidence categories
        top_sources = []
        for cat in categorized_data:
            if cat in confidence_scores and confidence_scores[cat] > 0.7:  # Only high-confidence categories
                top_sources += [r["link"] for r in categorized_data[cat][:2] if r.get("link")]
        profile["sources"] = list(set(top_sources))[:8]  # Remove duplicates and limit to 8
...
def save_startup_profile(profile: dict, output_dir: str = "data/startups"):
    ...
    # Build categorized_sources from raw_data
    raw_data = profile.get("raw_data", {})
    categorized_sources = {}
    for cat, items in raw_data.items():
        urls = list({item["link"] for item in items if item.get("link")})  # deduplicate
        categorized_sources[cat] = urls

    # Create final profile with the previous structure including metadata and categorized_sources
    final_profile = {
...
def main(startup_name: str | None = None, query_mode: str = "hybrid", enrich_content: bool = False):
    """
    ...
    if not startup_name:
        print("No startup name provided. Exiting.")
        return
    ...
    profile = generate_startup_profile(startup_name, data)
